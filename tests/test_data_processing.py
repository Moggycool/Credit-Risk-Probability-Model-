...
# Try to import helpers from src; if not present, provide lightweight fallbacks
try:
    from src.model_training import (
        prepare_data as _orig_prepare_data,
        evaluate_model,
        make_pipeline_with_scaler,
    )
...
def synthetic_df(n_customers=100, seed=0):
    ...
    # Drop identifier to prevent string/float conversion issues
    feat = feat.drop(columns=["CustomerId"])
    return feat
